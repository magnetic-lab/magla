...
class TestToolConfig(MaglaEntityTestFixture):
    ...
    def test_can_retieve_project(self, seed_tool_config):
        assert seed_tool_config.project.dict() == self.get_seed_data("Project", seed_tool_config.project.id-1)

    def test_can_retieve_tool_version(self, seed_tool_config):  
        assert seed_tool_config.tool_version.dict() == self.get_seed_data("ToolVersion", seed_tool_config.tool_version.id-1)

    def test_can_retieve_directory(self, seed_tool_config):
        assert seed_tool_config.directory.dict() == self.get_seed_data("Directory", seed_tool_config.directory.id-1)
        
    def test_can_retieve_tool(self, seed_tool_config):
        assert seed_tool_config.dict() == self.get_seed_data("ToolConfig", seed_tool_config.tool.id-1)
